...
# there are only 9 timescales here instead of 11 for adafactor mlp
...
class MuHyperV2(lopt_base.LearnedOptimizer):
  ...
  def __init__(
    ...
    self.identity_lstm_controls = identity_lstm_controls
    self.identity_lr_mult = identity_lr_mult
  ...
  def _rnn_forward(self, x, state):
    # for velo x is dim [#tensors, 30]
    if self.mix_layers:
      #NOT USED?? would map [#tensors, 30] --> [#tensors, 512]
      mix_layer = hk.Linear(self.lstm_hidden_size)(x)
      mix_layer = jax.nn.relu(mix_layer)

      #[#tensors, 30] --> [#tensors, 512]
      mix_layer = hk.Linear(self.lstm_hidden_size)(x)
      mix_layer = jax.nn.relu(mix_layer)
    ...
    #[#tensors, 512] --> [#tensors, 512], [#tensors, 512]
    rnn_out, state = self.lstm_fn()(x, state)
    
    #[#tensors, 512] --> [#tensors, param_inits]
    controls = hk.Linear(
        self.param_inits,
        name="rnn_to_controls",
    )(
  ...
  def _ff_mod(self, global_feat, extra_step_mult, p, g, m, rms, fac_g,
    ...
    # this differs from adafac original
    ...
    for wi, (w, b) in enumerate(zip(weights, biases)):
      if wi == 0:
        ...
        #[Ben] many small matmults instead of one large
  ...
  def opt_fn(self, theta, is_training=True) -> opt_base.Optimizer:
    ...
    class _Opt(opt_base.Optimizer):
      ...
      def update(self,
        ...
        if parent.identity_lr_mult:
          lr_mult = jnp.ones_like(lr_mult)
        ...
        def apply_one(control_param, key, lr_mult, p, g, m, rms, fac_g, v_col,
          ...
          def interpolate_theta(ff_p):
            ...
            if parent.identity_lstm_controls:
              c = jnp.ones_like(c)
          ...
          ff_param = jax.tree_util.tree_map(interpolate_theta,
                                            theta["ff_mod_stack"])
          # print("mup_lr_scale",jax.tree_util.tree_map(lambda x: x, mup_lr_scale))
          # jax.debug.print("mup_lr_scale {x} {y}",x=mup_lr_scale,y=key)
          # print("ff_param",jax.tree_util.tree_map(lambda x: x.shape, ff_param))
        ...
        # print("control_params",jax.tree_util.tree_map(lambda x: x.shape, control_params))
        # print("outer mup_lr_scale",jax.tree_util.tree_map(lambda x: x, mup_lr_scale))
        # print("outer ff_param",jax.tree_util.tree_map(lambda x: x.shape, theta["ff_mod_stack"]))
